Template.matscat.rendered = function(){
	var gl;
	var step = 0;
	...
	function init(gl){
		...
		if(!gl.renderer) {
			...
			var texture = THREE.ImageUtils.loadTexture( '/textures/blu.jpg', THREE.UVMapping, renderView );
	...
	function renderView(){
		...
		mesh.matrix.set(theta,0,0,0,
			0,theta,0,0,
			0,0,theta,0,
			0,0,0,1);
			mesh.matrixAutoUpdate = false;

			var elem=$("#scale-matrix");
			mm      = mesh.matrix.elements.map(function(x){return x});

			katex.render("\\begin{pmatrix}"+
			VM.Round(theta)+" & 0 & 0  \\\\ 0 & "+
			VM.Round(theta)+" & 0  \\\\	0 & 0 & "+
			VM.Round(theta)+"	\\end{pmatrix}",elem.get(0));
		}
	}
